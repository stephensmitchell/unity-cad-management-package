...
public class ComprehensiveControlShowcase2 : EditorWindow
{
    ...
    private void CreateDataViewsSection(VisualElement container)
    {
        ...
        // --- ListView Example (No changes needed here) ---
        ...
        var treeItems = new List<TreeViewItemData<string>>()
    {
        ...
        // 2. Create the TreeView with the makeItem and bindItem delegates
        var treeView = new TreeView(
            () => new Label(),
            (element, id) => (element as Label).text = treeItems.Find(item => item.id == id).data
        );
        ...
        // This automatically rebuilds the view.
